...
loader_configs = configs_worker.fetch_loader_configs(loader_name="FIVE_EXTERNAL_LEVEL")
...
trgr_dag = DAG(dag_id="daf_five_parallel_executor", start_date=pendulum.datetime(2022, 12, 11, tz="UTC"), catchup=False, schedule="@once")
...
for file_part in loader_configs['FILE_PARTS']:
  trigger_task = "trigger_five_external_{file_part}".format(file_part = file_part)
  trigger_dag = TriggerDagRunOperator(task_id=trigger_task, trigger_dag_id="daf_five_external_level", conf={"file_part": file_part})
